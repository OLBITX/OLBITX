...
{
    // Convert olbitx:// to olbitx:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("olbitx://"))
    {
        uri.replace(0, 12, "olbitx:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OLBITX.lnk";
}
...
{
    return GetAutostartDir() / "olbitx.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OLBITX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  olbitx-qt [" + tr("command-line options") + "]                     " + "\n";
